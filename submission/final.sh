...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '.vout[0].value' | awk '{printf "%.0f\n", $1*100000000}')
...
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r ".vout[$UTXO_VOUT_INDEX].value" | awk '{printf "%.0f\n", $1*100000000}')
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RBF=$(echo "$DECODED_TX" | jq -r 'if [.vin[].sequence] | any(. < 4294967294) then "true" else "false" end')
check_cmd "RBF verification" "VERIFY_RBF" "$VERIFY_RBF"

VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" '.vout[] | select(.scriptPubKey.address == $addr) | .value' | awk '{printf "%.8f\n", $1}')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address == $addr) | .value' | awk '{printf "%.8f\n", $1}')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$RAW_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address == $addr) | .n')
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX" | jq -r '.vout[0].value' | awk '{printf "%.0f\n", $1*100000000}')
